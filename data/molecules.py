...
class MoleculeDGLIndexed(torch.utils.data.Dataset):
...
class MoleculeDatasetIndexed(MoleculeDataset):
    def __init__(self, train_idx, val_idx, test_idx):
        self.name = "ZINC"
        start = time.time()
        print("[I] Loading dataset %s..." % (self.name))
        data_dir = "data/molecules/"
        with open(data_dir + self.name + ".pkl", "rb") as f:
            f = pickle.load(f)
            self._train = f[0]
            self._val = f[1]
            self._test = f[2]
        ...
        # print(f"true labels = {self._test.graph_labels}")

        with open(data_dir + "train.index", "r") as f:
            self._train_idx = sum([list(map(int, idx)) for idx in csv.reader(f)], [])
        with open(data_dir + "val.index", "r") as f:
            self._val_idx = sum([list(map(int, idx)) for idx in csv.reader(f)], [])
        with open(data_dir + "test.index", "r") as f:
            self._test_idx = sum([list(map(int, idx)) for idx in csv.reader(f)], [])

        graphs, labels = self._create_dataset_from_indexes(train_idx)
        self.train = MoleculeDGLIndexed("train", graphs, labels)
        graphs, labels = self._create_dataset_from_indexes(val_idx)
        self.val = MoleculeDGLIndexed("val", graphs, labels)
        graphs, labels = self._create_dataset_from_indexes(test_idx)
        self.test = MoleculeDGLIndexed("test", graphs, labels)
    ...
    def _create_dataset_from_indexes(self, indexes):
        def get_index(ll, idx):
            try:
                new_idx = ll.index(idx)
            except ValueError:
                return None
            return new_idx
        ...
        labels = []
        for idx in indexes:
            new_idx = get_index(self._train_idx, idx)
            if new_idx is not None:
                g, l = self._train[new_idx]
                dataset.append(g)
                labels.append(float(l))
                continue

            new_idx = get_index(self._val_idx, idx)
            if new_idx is not None:
                g, l = self._val[new_idx]
                dataset.append(g)
                labels.append(float(l))
                continue

            new_idx = get_index(self._test_idx, idx)
            if new_idx is not None:
                g, l = self._test[new_idx]
                dataset.append(g)
                labels.append(float(l))
                continue
